### Hexlet tests and linter status:
[![Actions Status](https://github.com/TaRgITay008/java-project-61/actions/workflows/hexlet-check.yml/badge.svg)](https://github.com/TaRgITay008/java-project-61/actions)
[![Quality Gate Status](https://img.shields.io/badge/Quality%20Gate-Passed-brightgreen?style=flat-square&logo=sonarqube&logoColor=white)](http://localhost:9000/dashboard?id=java-project-61)


## 🎮 Примеры запуска игр

### Меню выбора игры

![Меню](screenshots/menu.png)

### Игра "Проверка на чётность" - Победа

![Победа](screenshots/win.png)

### Игра "Проверка на чётность" - Поражение

![Поражение](screenshots/lose.png)


### Игра "Калькулятор" - Победа

![Калькулятор Победа](screenshots/calc-win.png)

### Игра "Калькулятор" - Поражение

![Калькулятор Поражение](screenshots/calc-lose.png)


### Игра "НОД" - Победа

![НОД Победа](screenshots/gcd-win.png)

### Игра "НОД" - Поражение

![НОД Поражение](screenshots/gcd-lose.png)


### Игра "Арифметическая прогрессия" - Победа

![Прогрессия Победа](screenshots/progression-win.png)

### Игра "Арифметическая прогрессия" - Поражение

![Прогрессия Поражение](screenshots/progression-lose.png)
